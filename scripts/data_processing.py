...
def clean_data(df, dashboard_file_pre):
    ...
    df, columns_to_drop = drop_columns_with_missing_data(df=df, nan_data_threshold_perc=0.5)
...
def feature_engineering_steps(df):
    ...
    df, additional_columns = apply_feature_engineering(df=df, add_dummy_data=True)
    ...
    if target_class_str in df.columns:
        ...
        logging.info(
            f"Encoded target variable '{target_class}' with value counts:\n{df[target_class].value_counts()}"
        )
    else:
