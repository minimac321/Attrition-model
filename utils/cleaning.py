...
def fill_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    categorical_imputer = SimpleImputer(strategy='most_frequent')  # Use most frequent value for categorical columns
    ...
    df_filled = pd.DataFrame(preprocessor.fit_transform(df), columns=numeric_columns.tolist() + categorical_columns.tolist())
...
def modified_z_score_anomaly_detection(df: pd.DataFrame, column: str, threshold: float = 3.5) -> pd.DataFrame:
    """
