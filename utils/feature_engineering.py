...
def add_random_survey_fields(df: pd.DataFrame, seed_value: int) -> pd.DataFrame:
    ...
    df["EmployeeEngagementLevel_dummy"] = np.random.choice(
        engagement_levels, df.shape[0], p=[0.4, 0.4, 0.2]
    )

    # Adding a 'Training Opportunity' availability (boolean)
    train_opportunities_levels = [True, False]
    df["TrainingOpportunitiesBool_dummy"] = np.random.choice(
        train_opportunities_levels, df.shape[0], p=[0.4, 0.6]
    )

    fields_added = [
        "WorkLifeBalanceScore_dummy",
        "WorkPlaceSatisfactionScore_dummy",
...
def years_in_role_to_years_at_company(df: pd.DataFrame) -> pd.DataFrame:
    df["PercentOfYearsInCurrentRole"] = np.where(
        df["YearsAtCompany"] != 0, df["YearsInCurrentRole"] / df["YearsAtCompany"], np.nan
    )
    return df
...
def years_since_promotion_to_years_at_company(df: pd.DataFrame) -> pd.DataFrame:
    df["YearsSincePromoOverTotalYearsRatio"] = np.where(
        df["YearsAtCompany"] != 0, df["YearsSinceLastPromotion"] / df["YearsAtCompany"], np.nan
    )
    return df
...
def income_percent_of_department_avg(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["IncomeToDeptAvg"] = np.where(
        df["DeptAvgIncome"] != 0, df["MonthlyIncome"] / df["DeptAvgIncome"], np.nan
    )
...
def overtime_job_satisfaction_interaction(df: pd.DataFrame) -> pd.DataFrame:
    df["OvertimeJobSatisfactionInteraction"] = (
        df["OverTime"] + "_" + df["JobSatisfaction"].astype(str)
    )
    return df
...
def years_company_performance_interaction(df: pd.DataFrame) -> pd.DataFrame:
    df["YearsAtCompanyPerformanceInteraction"] = df["YearsAtCompany"] * df[
        "PerformanceRating"
    ].astype(float)
    return df
...
def performance_rating_job_satisfaction_interaction(df: pd.DataFrame) -> pd.DataFrame:
    df["OvertimeJobSatisfactionInteraction"] = (
        df["PerformanceRating"] + "_" + df["JobSatisfaction"].astype(str)
    )
    return df
...
def safe_feature_engineering(
    feature_func: callable, df: pd.DataFrame, required_columns: Optional[list] = None
) -> pd.DataFrame:
    try:
...
def apply_feature_engineering(df: pd.DataFrame, add_dummy_data: bool = True) -> pd.DataFrame:
    ...
    df = safe_feature_engineering(
        years_in_role_to_years_at_company, df, ["YearsInCurrentRole", "YearsAtCompany"]
    )
    df = safe_feature_engineering(
        years_since_promotion_to_years_at_company, df, ["YearsSinceLastPromotion", "YearsAtCompany"]
    )
    df = safe_feature_engineering(
        salary_hike_to_monthly_income, df, ["PercentSalaryHike", "MonthlyIncome"]
    )
    ...
    df = safe_feature_engineering(
        satisfaction_to_department_avg, df, ["JobSatisfaction", "Department"]
    )
    ...
    df = safe_feature_engineering(
        marital_status_gender_interaction, df, ["MaritalStatus", "Gender"]
    )
    df = safe_feature_engineering(
        overtime_job_satisfaction_interaction, df, ["OverTime", "JobSatisfaction"]
    )
    df = safe_feature_engineering(
        years_company_performance_interaction, df, ["YearsAtCompany", "PerformanceRating"]
    )
    df = safe_feature_engineering(
        performance_rating_job_satisfaction_interaction,
        df,
